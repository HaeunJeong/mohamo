...
export class MatchingTimePage {
  ...
  addNewMeetingInfo() {
    ...
    firebase.database().ref('/allMeeting/' + this.meeting_code + '/infoToMeet/' + new_time).child('attendanceLog').set(true)
      .then(result => {
        firebase.database().ref('/allMeeting/' + this.meeting_code + '/member').once('value', (snapshot) => {
          snapshot.forEach(snap=>{
            firebase.database().ref('/allMeeting/' + this.meeting_code + '/infoToMeet/' + new_time+'/attendanceLog').child(snap.key).set(false);
            return false;
          })
        });

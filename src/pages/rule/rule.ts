...
*/
  ...
  {//firebase에서 데이터 불러오기(총벌점, 지각, 지각벌점, 조퇴벌점, 무단결석벌점)
    ...
    firebase.database().ref('/allMeeting/' + this.meetingCode +'/setting_rate').once('value').then(function(snap){
    ...
    this.setting_rate = late_setting_temp;
    ...
    this.penalty_rate = late_penalty_temp;
  ...
  {
    firebase.database().ref('/allMeeting/' + this.meetingCode ).child('setting_rate').push(selectedValue);
